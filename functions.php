...
add_action('init', function () {
	...
	// Taxonomy registration belongs in a plugin.
...
add_action('save_post', function ($post_id) {
	...
	if (get_post_type($post_id) !== 'mptbm_rent') return;
...
function add_location_image_field() { ?>
	<div class="form-field term-group">
		...
		<button type="button" class="button button-secondary" id="upload-image-btn">Upload Image</button>
	</div>
	<?php
}
add_action('locations_add_form_fields', 'add_location_image_field');

function edit_location_image_field($term) {
	$image_id = get_term_meta($term->term_id, 'location-image', true);
	?>
	<tr class="form-field term-group-wrap">
		<th scope="row"><label for="location-image">Image</label></th>
		<td>
			<?php wp_nonce_field('urbantaxi_location_image_action', 'urbantaxi_location_image_nonce'); ?>
...
add_action('locations_edit_form_fields', 'edit_location_image_field');

function save_location_image($term_id) {
	if (
	...
	if (isset($_POST['location-image'])) {
		update_term_meta($term_id, 'location-image', sanitize_text_field(wp_unslash($_POST['location-image'])));
	}
}
add_action('created_locations', 'save_location_image');
add_action('edited_locations', 'save_location_image');

function location_admin_scripts($hook) {
...
add_action('admin_footer', 'location_admin_scripts');
// location image end 


// single transportation start
...
// single transportation end

// booking form scroll issue start
            ...
            if ($scrollableParent) {
                ...
                if ($modalContent.length) {
                    $modalContent.animate({
                        scrollTop: $modalContent[0].scrollHeight
                    }, 500);
                }
            }
            return false;
...
// booking form scroll issue end

// team edit with elementor start
...
// team edit with elementor end

// for booking form labels start 
...
// for booking form labels end
// for Checkout Image Display start
add_action('init', function () {
...
// for Checkout Image Display end
